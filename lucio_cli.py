...
def adaptive_optimization(doc_count: int):
    ...
    if doc_count <= 20:
        # Small corpus: extract EVERYTHING
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC, MAX_PDF_PAGES = 500, 100, 30, 80
    elif doc_count <= 50:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC, MAX_PDF_PAGES = 450, 90, 20, 60
    elif doc_count <= 100:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC, MAX_PDF_PAGES = 400, 70, 12, 40
    elif doc_count <= 200:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC, MAX_PDF_PAGES = 350, 60, 8, 30
    else:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC, MAX_PDF_PAGES = 300, 50, 6, 25
...
def extract_numbers(text: str) -> Set[str]:
    ...
    nums = set()
    # Match currency amounts, percentages, plain numbers
...
def extract_key_terms(text: str) -> Set[str]:
    ...
    # ALL-CAPS words (like legal names: BLACKMUN, REHNQUIST)
...
def score_answer(predicted: str, ground_truth: str) -> dict:
    ...
    # 4. Number matching — critical for financial/quantitative questions
    ...
    # 5. Key term matching — for names, entities, legal terms
    ...
    if exact:
        ...
        # Weighted: token F1 + number accuracy + key term accuracy
        composite = max(
            f1,
...
def _extract_spreadsheet(fp: Path) -> str:
    # Try openpyxl first, then pandas fallback
    text_parts = []
    try:
        import openpyxl
...
def process_file(fp: Path) -> List[str]:
    try:
        ...
        text = clean_text(raw)
        fname = fp.stem.replace("_", " ").replace("-", " ")
        full = f"[Source: {fname}]\n{text}"
        return chunk_text(full, CHUNK_SIZE, CHUNK_OVERLAP)[:MAX_CHUNKS_PER_DOC]
    except Exception as e:
...
def hybrid_retrieve(bm25, chunks, query, top_k=10, bm25_k=80,
    ...
    # Extract matching signals from query
    q_proper = set()
...
def ingest():
    ...
    bm25 = BM25Index(); bm25.build(all_chunks)
    vec = TFIDFVectorizer(bm25, min_df=2)
    mat = vec.vectorize_documents(all_chunks)
    ...
    try:
        import faiss; idx = faiss.IndexFlatIP(384)
        idx.add(np.zeros((1,384), dtype='float32')); faiss.write_index(idx, INDEX_PATH)
    except: pass
...
def run():
    ...
    for q in questions:
        ...
        retrieval.append({"q": q, "ctx": ctx, "prompt": build_qa_prompt(q, ctx)})
    ...
    for i, q in enumerate(questions):
        ans = answers[i] or "Information not found in provided documents"
        src = "gemini" if answers[i] else "no_answer"
        ...
        final.append({"question": q, "answer": ans, "ground_truth": gt_s, "source": src, "scores": sc})
