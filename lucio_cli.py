...
def adaptive_optimization(doc_count: int):
    ...
    if doc_count <= 20:
        ...
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC = 400, 80, 5
        SKIP_DOMAIN_EXTRACTION = False
    elif doc_count <= 200:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC = 350, 60, 4
        SKIP_DOMAIN_EXTRACTION = True
    else:
        CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHUNKS_PER_DOC = 250, 40, 3
        SKIP_DOMAIN_EXTRACTION = True
    logger.info(f"Adaptive: {doc_count} docs -> chunk={CHUNK_SIZE}, overlap={CHUNK_OVERLAP}, "
                f"max_chunks/doc={MAX_CHUNKS_PER_DOC}, skip_domain={SKIP_DOMAIN_EXTRACTION}")
...
# QUERY EXPANSION
# ==============================================================================
_KEYWORD_EXPANSIONS: Dict[str, set] = {
    "revenue": {"sales", "income", "earnings", "profit", "financial", "results",
                "billion", "million", "quarter", "annual", "growth", "net", "gross",
                "operating", "total", "fiscal", "year", "period", "increased",
                "decreased", "compared", "statement", "consolidated"},
    "executive": {"ceo", "cfo", "coo", "cto", "president", "officer", "director",
                  "management", "leadership", "chief", "board", "vice", "senior",
                  "head", "named", "appointed", "age", "since", "joined", "serves"},
    "key": {"ceo", "cfo", "coo", "cto", "president", "officer", "director",
            "management", "leadership", "chief", "executive", "named", "appointed",
            "senior", "age", "biography", "background"},
    "risk": {"uncertainty", "threat", "challenge", "adverse", "litigation",
             "regulatory", "compliance", "volatility", "exposure", "material",
             "forward-looking", "could", "uncertain", "fluctuation", "subject",
             "assurance", "factor", "affect", "impact", "significant"},
    "business": {"operations", "platform", "services", "industry", "sector",
                 "commercial", "enterprise", "strategic", "segment", "product",
                 "market", "customer", "solution", "technology", "company",
                 "founded", "headquartered", "provides", "offers", "develops",
                 "principal", "core", "overview", "description"},
    "earnings": {"quarter", "q1", "q2", "q3", "q4", "guidance", "forecast",
                 "projection", "conference", "call", "results", "financial",
                 "report", "analyst", "prepared", "remarks", "operator"},
    "call": {"conference", "earnings", "quarter", "transcript", "analyst",
             "investor", "remarks", "guidance", "prepared", "operator", "question"},
    "legal": {"litigation", "lawsuit", "court", "proceedings", "regulatory",
              "compliance", "settlement", "judgment", "investigation"},
    "acquisition": {"merger", "purchase", "deal", "transaction", "consolidation",
                    "buyout", "takeover"},
    "dividend": {"payout", "distribution", "yield", "shareholders", "returns",
                 "buyback", "repurchase"},
    "competitor": {"competition", "rival", "peer", "market", "landscape", "share"},
    "strategic": {"strategy", "plan", "goal", "roadmap", "initiative", "priority",
                  "growth", "vision"},
    "stock": {"share", "equity", "securities", "price", "trading", "performance"},
    "bench": {"court", "judge", "justice", "justices", "opinion", "dissent",
              "decided", "argued", "supreme", "scotus", "bench"},
    "scotus": {"supreme", "court", "justice", "opinion", "decided", "argued",
               "certiorari", "petitioner", "respondent", "bench"},
    "governing": {"law", "jurisdiction", "applicable", "governed", "provision",
                  "agreement", "section", "clause", "contract"},
    "notify": {"notification", "filing", "threshold", "approval", "clearance",
               "merger", "combination", "antitrust", "competition"},
    "anticompetitive": {"competition", "merger", "combination", "dominance",
                        "market", "threshold", "turnover", "assets", "aaec",
                        "appreciable", "adverse", "effect"},
    "cci": {"competition", "commission", "india", "merger", "combination",
            "threshold", "turnover", "assets", "notification", "act"},
    "turnover": {"revenue", "sales", "assets", "threshold", "crore", "filing",
                 "notification", "combination"},
    "nvca": {"investor", "rights", "agreement", "venture", "capital", "ira",
             "registration", "preferred", "stockholder", "board", "covenant"},
    "ira": {"investor", "rights", "agreement", "registration", "information",
            "covenant", "board", "observer", "preferred"},
    "kodak": {"eastman", "kodak", "antitrust", "tying", "market", "power",
              "equipment", "service", "parts", "competition"},
    "meta": {"facebook", "meta", "platforms", "social", "media", "advertising",
             "reality", "labs", "family", "apps"},
    "kfin": {"kfintech", "kfin", "registrar", "transfer", "agent", "fintech",
             "mutual", "fund"},
}
...
def _detect_query_category(question: str) -> str:
    ql = question.lower()
    if any(w in ql for w in ["revenue", "sales", "income", "profit", "financial results", "net income"]):
        return "revenue"
    if any(w in ql for w in ["executive", "officer", "ceo", "cfo", "director", "leadership", "management team", "key people"]):
        return "executive"
    if any(w in ql for w in ["risk", "uncertainty", "threat", "challenge", "adverse"]):
        return "risk"
    if any(w in ql for w in ["earnings call", "conference call", "quarterly call", "prepared remarks"]):
        return "earnings"
    if any(w in ql for w in ["business", "operations", "company do", "products", "services", "core", "overview"]):
        return "business"
    return "general"


_TAG_BOOSTS = {
    "revenue": {"[FINANCIAL TABLE]": 0.18, "[EARNINGS": 0.06},
    "executive": {"[EXECUTIVE]": 0.22},
    "risk": {},
    "business": {},
    "earnings": {"[EARNINGS CALL": 0.15, "[EARNINGS GUIDANCE]": 0.10},
}
